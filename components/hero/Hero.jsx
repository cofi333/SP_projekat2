...
const Hero = () => {
  ...
  const handleDrop = (draggedSweater) =>
    setSweaters((prevSweaters) =>
      prevSweaters.filter((item) => item.id !== draggedSweater),
    );
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        "https://sheet.best/api/sheets/23c3bb4f-1443-485a-94a8-0ed3f7f04930",
      );
  ...
  const sendData = async () => {
    if (data.length > 0) {
      ...
      const lastSentDateTime = parseDateTime(
        lastRequest.date,
        lastRequest.time,
      );
      ...
      if (lastSentDateTime && currentDateTime - lastSentDateTime < 600000) {
        setSnackBar({
          open: true,
          message:
            "Error sending data. 10 minutes haven't passed since the last request.",
          type: "error",
        });
        return;
      }
    ...
    try {
      if (!isSending) {
        setIsSending(true);
        const data = {
          szent_istvan_kiraly_zenei_alapitvany:
            numOfSweaters[0] === 0 ? "0" : numOfSweaters[0],
          autizmus_alapitvany: numOfSweaters[1] === 0 ? "0" : numOfSweaters[1],
          elemiszer_bankegysulet:
            numOfSweaters[2] === 0 ? "0" : numOfSweaters[2],
  ...
  const addBackSweater = (sweaterId) => {
    let addSweater = sweaters.find((sweater) => sweater.id === sweaterId);
    setSweaters((prevSweaters) => {
  ...
  const { isOver, setNodeRef } = useDroppable({
    id: "hatstand",
    data: {
      addBackSweater: addBackSweater,
      removeSweaterFromShelf: removeSweaterFromShelf,
    },
  });
  ...
  return (
    <div>
      <div className="hero-header">
        <Image
          src="/assets/headerImage2022.png"
          width={0}
          height={0}
          sizes="100vw"
      ...
      <div className="hero-hat-stand" ref={setNodeRef}>
        <Image
          src="/assets/hat_stand.png"
          width={0}
          height={0}
          sizes="100vw"
          style={{ width: "100%", height: "auto" }}
        ...
        {displaySendBtn && (
          <div id="hero-sendBtn">
            <Button
              text={isSending ? "Sending..." : "ELKŰLDŐM"}
              onClick={sendData}
              disabled={isSending}
            />
          </div>
      ...
      <div className="hero-shelfs">
        {foundations.map((foundation, index) => (
          <Shelf
            key={foundation.id}
            id={foundation.id}
            onChange={handleChildState}
            name={foundation.name.toUpperCase()}
